<?php
// index.php
// This file displays either the login or signup form based on URL parameter,
// and handles displaying messages passed via URL parameters.

// Start the session. This is needed to check for user sessions (even if just for redirection).
session_start();

// Determine which form to show based on a URL parameter, default to login
$form_to_show = isset($_GET['form']) ? $_GET['form'] : 'login';
// echo $form_to_show; // Debugging line to check if this file is being executed

// Initialize variables for messages
$error_message = '';
$success_message = '';

// Check for error messages from the URL (CORRECTED: Using $_GET)
if (isset($_GET['error'])) {
    $error_code = $_GET['error']; // CORRECTED: Using $_GET
    switch ($error_code) {
        case 'emptyfields':
        ...
        case 'dbconnection':
            ...
            // Avoid showing raw database errors to the user for security
            break;
        case 'preparefailed':
            $error_message = 'An application error occurred (prepare failed). Please try again later.';
            // Avoid showing raw database errors
            break;
        ...
        case 'incorrectpassword':
            $error_message = 'Incorrect email or password.'; // Generic message for security
            break;
        case 'usernotfound':
            $error_message = 'Incorrect email or password.'; // Generic message for security
            break;
...
// Check for success messages from the URL (CORRECTED: Using $_GET)
if (isset($_GET['success'])) {
    $success_code = $_GET['success']; // CORRECTED: Using $_GET
    switch ($success_code) {
        case 'registered':
            $success_message = 'Registration successful! Please log in.';
            break;
        case 'signedup_placeholder': // Placeholder success message (if you used this in process_signup)
            $success_message = 'Sign up successful (placeholder). Please log in.';
            break;
        // Add other success message types if needed
        default:
...
// Initialize email variable for login form to retain value on error
// If the processing script redirects back with the email in the URL (e.g., ?email=...),
// you can retrieve it here. Otherwise, it will be empty on error.
    ...
    <div class="container flex flex-col md:flex-row w-full max-w-4xl bg-[#1f1f1f] rounded-lg overflow-hidden shadow-lg">
        ...
        <div class="form-section flex-1 p-8 md:p-10 flex flex-col justify-center">
            ...
            <?php
                ...
                // These are displayed here in index.php, based on $_GET parameters.
                if ($success_message) {
                    echo "<div class='bg-green-500 text-white p-3 rounded mb-4 text-center'>" . htmlspecialchars($success_message) . '</div>';
                }
                if ($error_message) {
                    // Using bg-red-600 for better contrast on dark background
